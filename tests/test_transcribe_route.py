...
def test_transcribe_streaming_returns_400(app_client, data_plane_headers):
    body = {"audio_b64": base64.b64encode(b"\x00\x00").decode(), "mime": "audio/wav", "prefer": "streaming"}
    r = app_client.post("/v1/transcribe", json=body, headers=data_plane_headers())
    assert r.status_code == 400


def test_transcribe_bad_base64_returns_400(app_client, data_plane_headers):
    r = app_client.post(
        "/v1/transcribe", json={"audio_b64": "!!!not-base64!!!", "mime": "audio/wav"}, headers=data_plane_headers()
    )
    assert r.status_code in (400, 502)


def test_transcribe_default_calls_registered_provider(app_client, data_plane_headers):
...
def test_transcribe_provider_error_becomes_502(app_client, data_plane_headers):
...
def test_transcribe_stub_returns_501(app_client, data_plane_headers):
...
def test_transcribe_without_credential_is_401(app_client):
    body = {"audio_b64": base64.b64encode(b"\x00").decode(), "mime": "audio/wav", "prefer": "default"}
